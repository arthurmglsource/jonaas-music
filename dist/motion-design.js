...
function draw(){scheduled=false;
 if(reduce.matches){[stage,catalogueStage,music,poster,info,contact,portrait,...cards].forEach(e=>{e.style.transform='';e.style.clipPath='';e.style.opacity='';e.style.visibility='';e.inert=false});$('.origin').style.setProperty('--origin-progress','1');$('.origin-stars').style.transform='none';return;}
 ...
 if(mobile.matches){[poster,info,music,catalogueStage].forEach(e=>{e.style.transform='';e.style.opacity='';e.style.visibility='';e.style.clipPath='';e.inert=false})}
 else{poster.style.transform=`translateX(${(1-enter)*27}%) scale(${1+(1-enter)*.025})`;poster.style.opacity='1';poster.inert=false;info.style.transform=`translateX(${(1-enter)*45}px)`;info.style.opacity=String(enter);info.style.visibility=enter<.01?'hidden':'visible';info.inert=enter<.1;music.style.clipPath=`inset(0 ${exit*100}% 0 0)`;music.style.transform=`translateX(${-exit*9}%)`;music.inert=exit>.98;const catTop=catalogue.getBoundingClientRect().top;catalogueStage.style.transform=mp>.6&&catTop>0?`translateY(${-catTop}px)`:'';}
 ...
 const origin=$('.origin'),or=origin.getBoundingClientRect(),op=clamp((innerHeight-or.top)/(innerHeight+or.height));portrait.style.transform=`translateY(${(op-.5)*-24}px)`;origin.style.setProperty('--origin-progress',op.toFixed(3));$('.origin-stars').style.transform=`rotate(${-12+op*20}deg) translateY(${(1-op)*15}px)`;
}
